...
var (
	Q =new(Query)
	{{range $name,$d :=.Data -}}
	{{$d.ModelStructName}}Field *{{$d.QueryStructName}}
	{{end -}}
)
...
func SetDefault(db *gorm.DB) {
	*Q = *Use(db)
	{{range $name,$d :=.Data -}}
	{{$d.ModelStructName}}Field = &Q.{{$d.ModelStructName}}
	{{end -}}
}
...
func Use(db *gorm.DB) *Query {
	return &Query{
		db: db,
		{{range $name,$d :=.Data -}}
		{{$d.ModelStructName}}Field: new{{$d.ModelStructName}}(db),
		{{end -}}
	}
}
...
func (q *Query) clone(db *gorm.DB) *Query {
	return &Query{
		db: db,
		{{range $name,$d :=.Data -}}
		{{$d.ModelStructName}}Field: q.{{$d.ModelStructName}}.clone(db),
		{{end}}
	}
}
...
type queryCtx struct{ 
	{{range $name,$d :=.Data -}}
	{{$d.ModelStructName}}Query {{$d.ReturnObject}}
	{{end}}
}
...
func (q *Query) WithContext(ctx context.Context) *queryCtx  {
	return &queryCtx{
		{{range $name,$d :=.Data -}}
		{{$d.ModelStructName}}Query: q.{{$d.ModelStructName}}.WithContext(ctx),
		{{end}}
	}
}
